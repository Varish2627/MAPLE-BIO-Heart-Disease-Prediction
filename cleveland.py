...
class MED_CARE:
    ...
    def balance_data(self, df, target_col):
        ...
        # Feature engineering can introduce NaNs (for example, from invalid
        # divisions). BIO-SYN must receive finite training values.
...
class CLARITY_OD:
    ...
    def detect_and_treat_train(self, df, target_col):
        ...
        if outlier_mask.any():
            ...
            # Use positional values so pandas broadcasts the feature medians
            # to every selected row instead of aligning the Series to row IDs.
...
for csv_path, target_col in datasets:
        ...
        print("\nCleveland ablation study metrics:")
        print(pd.DataFrame(METRICS.Cleveland_Ablation_Study).to_string(index=False))
import Graph 
